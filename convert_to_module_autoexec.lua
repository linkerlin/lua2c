...
-- 转换 lua2c 生成的 C 代码为自动执行的 Lua 模块
-- 用法: lua convert_to_module_autoexec.lua input.c output.c module_name
...
if not input_file or not output_file or not module_name then
  print("用法: lua convert_to_module_autoexec.lua input.c output.c module_name")
  os.exit(1)
end
...
-- 修复 LUA_ENVIRONINDEX 的使用，替换为 LUA_GLOBALSINDEX
content = content:gsub("LUA_ENVIRONINDEX", "LUA_GLOBALSINDEX")
...
]], entry_function_name, simple_module_name, simple_module_name, simple_module_name)
...
print("已将 " .. input_file .. " 转换为自动执行的 Lua 模块 " .. output_file)
